...
x_position = 0
y_position = 0
s_last_speed = 0  # s is for solena
p_last_speed = 0  # p is for paige
s_position = 0
p_position = 0
    ...
    for event in pygame.event.get():
        ...
        if event_dict.get("joy") == 0:  # full stop button
            ...
            if event_dict.get("button") == 0:
                ...
                writeToSerial(str(int(c_recent_speed)) + "e")
                writeToSerial(str(int(d_recent_speed)) + "f")

        if event_dict.get("button") == 10:  # laser
            writeToSerial(
                str("10l")
            )  # 10 doesnt matter just needs to be there because code expects it
        ...
        if (
            event_dict.get("axis") == 2
        ):  # thruster's going forward to backward (right joystick x axis)
            x_position = event.dict.get("value")
            if int(x_position) != x_last_speed:
                x_last_speed = x_position

        if (
            event_dict.get("axis") == 3
        ):  # thruster's going up and down (right joystick y axis)
        ...
        if (
            event_dict.get("axis") == 1
        ):  # thruster's pitching up and down (left joystick y axis)
            p_position = event.dict.get("value")
            if int(p_position) != p_last_speed:
                p_last_speed = p_position

        if (
            event_dict.get("axis") == 0
        ):  # thruster's yawing left and right (left joystick x axis)
        ...
        if (
            abs(x_position) > abs(y_position)
            and abs(x_position) > abs(p_position)
            and abs(x_position) > abs(s_position)
        ):  # x should be preferred -forward & backward
            speed_forward = myround(
                translate(x_position, -1, 1, speed_fb_min, speed_fb_max)
            )  # forward
            if int(speed_forward) != c_recent_speed:
                ...
                if current_time > last_send_time + SEND_EVERY:
                    last_send_time = current_time
                    writeToSerial(
                        str(int(speed_forward)) + "c"
                    )  # c is forward thruster
                    c_recent_speed = speed_forward

            # was previously [1400, 1500]
            speed_backward = myround(
                translate(x_position, 1, -1, speed_fb_max, speed_fb_min)
            )  # backward
            if int(speed_backward) != d_recent_speed:
                ...
                if current_time > last_send_time + SEND_EVERY:
                    last_send_time = current_time
                    writeToSerial(
                        str(int(speed_backward)) + "d"
                    )  # d is backward thruster
                    d_recent_speed = speed_backward
            ##########################################################################################################
            speed_down = myround(
                translate(abs(x_position), 0, 1, STOP_INT, speed_fb_d_max)
            ...
            if int(speed_down) != b_recent_speed:
                ...
                if current_time > last_send_time + SEND_EVERY:
                    ...
                    writeToSerial(str(int(speed_down)) + "f")

            speed_up = myround(
                translate(abs(x_position), 0, 1, STOP_INT, speed_fb_u_min)
            )  # between 1500 and 1400
            if int(speed_up) != a_recent_speed:
                ...
                if current_time > last_send_time + SEND_EVERY:
                    ...
                    writeToSerial(str(int(speed_up)) + "e")
        ...
        ###########################################################################################################
            ...
            abs(y_position) > abs(x_position)
            and abs(y_position) > abs(p_position)
            and abs(y_position) > abs(s_position)
        ):  # if y is greater than x (up and down)

            speed_up_1 = myround(
                translate(y_position, -1, 1, speed_ud_max, speed_ud_min)
            )  # up
            if int(speed_up_1) != a_recent_speed:
                ...
                if current_time > last_send_time + SEND_EVERY:
                    ...
                    writeToSerial(str(int(speed_up_1)) + "e")

            speed_down_1 = myround(
                translate(y_position, 1, -1, speed_ud_min, speed_ud_max)
            )  # down
            if int(speed_down_1) != b_recent_speed:
                ...
                if current_time > last_send_time + SEND_EVERY:
                    ...
                    writeToSerial(str(int(speed_up_1)) + "f")
            ...
            abs(p_position) > abs(s_position)
            and abs(p_position) > abs(x_position)
            and abs(p_position) > abs(y_position)
            and abs(p_position) > 0.05
        ):  # x should be preferred (turn right(yaw))

            speed_up_1 = myround(
                translate(p_position, -1, 1, 1600, 1400)
            )  # pitch up (FORMERLY 1300-1700 AS OF 10/6/25)
            if int(speed_up_1) != a_recent_speed:
                current_time = time.time()
                if current_time > last_send_time + SEND_EVERY:
                    last_send_time = current_time
            ...
            speed_down_1 = myround(
                translate(p_position, 1, -1, 1600, 1400)
            )  # pitch down (FORMERLY 1300-1700 AS OF 10/6/25)
            if int(speed_down_1) != b_recent_speed:
                current_time = time.time()
                if current_time > last_send_time + SEND_EVERY:
                    last_send_time = current_time
                    writeToSerial(str(int(speed_down_1)) + "b")
                    b_recent_speed = speed_down_1
                    writeToSerial(str(int(speed_down_1)) + "f")
            ...
            abs(s_position) > abs(p_position)
            and abs(s_position) > abs(x_position)
            and abs(s_position) > abs(y_position)
            and abs(s_position) > 0.05
        ):  # x should be preferred (turn right(yaw))
            ...
            speed_forward = myround(
                translate(s_position, -1, 1, 1400, 1600)
            )  # turn right (FORMERLY 1300-1700 AS OF 10/6/25)
            if int(speed_forward) != c_recent_speed:
                current_time = time.time()
                if current_time > last_send_time + SEND_EVERY:
                    last_send_time = current_time
            ...
            speed_backward = myround(
                translate(s_position, 1, -1, 1400, 1600)
            )  # turn left (FORMERLY 1300-1700 AS OF 10/6/25)
            if int(speed_backward) != d_recent_speed:
                current_time = time.time()
                if current_time > last_send_time + SEND_EVERY:
                    last_send_time = current_time
